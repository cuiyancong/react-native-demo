...
Tile.prototype.hasMoved = function() {
  return (this.fromRow() !== -1 && (this.fromRow() !== this.toRow() || this.fromColumn() !==
      this.toColumn())) ||
    this.mergedInto;
};
...
Board.prototype.moveLeft = function() {
  ...
  for (var row = 0; row < Board.size; ++row) {
    ...
    for (var target = 0; target < Board.size; ++target) {
      ...
      hasChanged = hasChanged || (targetTile.value !== this.cells[row][target]
        .value);
    }
  ...
  for (var row = 0; row < Board.size; ++row) {
    for (var column = 0; column < Board.size; ++column) {
      ...
      for (var dir = 0; dir < 4; ++dir) {
        ...
        if (newRow < 0 || newRow >= Board.size || newColumn < 0 || newColumn >=
          Board.size) {
          continue;
        }
        canMove = canMove || (this.cells[row][column].value === this.cells[
